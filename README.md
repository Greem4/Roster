...
| Где | Что | Команда |
|-----|-----|---------|
| **Mac** | фронт + API + БД (всё локально) | `./scripts/dev.sh` |
| **GitHub** | после `git push` в `release` | GitHub Actions → Pi |
| **Pi** | свой код + **своя БД** (данные не с Mac) | автоматически |
...
Скрипт сам поднимает Docker (PostgreSQL + API) и Vite UI → http://localhost:5173.

**Первый раз:** скопировать `.env` с Pi и `./scripts/internal/sync-db-from-pi.sh` (данные для локальной разработки).

### После push — автоматически на Pi

Рабочие ветки (`автографик`, `main`, …) — только разработка. На прод уходит **ветка `release`**:

```bash
git checkout release
git merge автографик          # или cherry-pick / PR
git push origin release       # → GitHub Actions → Pi
```

GitHub Actions (`.github/workflows/deploy-pi.yml`) выкладывает **код** API и фронта на малинку. БД на Pi **не перезаливается** — при старте api Alembic накатывает только новые миграции.

**Один раз:** в GitHub → Settings → Secrets → `ROSTER_SSH_KEY` (приватный SSH-ключ с доступом к VPS и Pi).

Ручной деплой (если нужен до push):

```bash
./scripts/deploy.sh
```

Перенос **данных** Mac → Pi — только явно: `./scripts/internal/sync-db-to-pi.sh --confirm` (обычно не нужно).

Cursor-агент после правок **API** может запустить `deploy.sh --backend` для быстрой проверки; **фронт на сайт** — только по запросу.

**Один раз дома** — см. [SSH на Pi](#ssh-на-pi-дома-и-вне-дома) и [Mac: команды → setup](#mac-команды).

**Вне дома** (кафе, LTE): SSH на Pi — **`ssh roster-pi-remote`**, не `greem4@192.168.31.96`. Деплой — `./scripts/deploy.sh` (маршрут auto). Локальная разработка — `./scripts/dev.sh` (Docker на Mac). Подробно — ниже ↓
...
| Деплой | `./scripts/deploy.sh` | то же (auto: LAN или VPS) |
| Туннель PostgreSQL | `./scripts/internal/tunnel-db.sh` | то же |
| Локальный UI | `./scripts/dev.sh` | `./scripts/dev.sh` (fallback: prod API) |
| Локальный бэкенд | `./scripts/internal/dev-stack.sh up` | то же (нужен Docker Desktop) |
| Данные Pi → Mac | `./scripts/internal/sync-db-from-pi.sh` | то же (auto: LAN или VPS) |
...
Новые лекарства: JSON в консоли SQL — `SELECT rx_import_medicines('…'::jsonb);` (шаблон **`scripts/internal/data/rx-import.sql`**). Только добавление; удаления в БД сохраняются. **`import.sh` не использовать** — архивный JSON.

**Mac (рекомендуется):** полный перенос с Pi одной командой:

```bash
./scripts/internal/sync-db-from-pi.sh
```

Для ручного `pg_dump` (порт 5432 должен быть открыт, например после tunnel-db.sh):
...
6. На новой Pi: `~/server` + `./scripts/setup/caddy.sh` + `./scripts/deploy.sh --frontend`, туннель Pi→VPS, `./scripts/setup/docker-autostart.sh`.
...
| `~/server/www` | статика фронта | `./scripts/deploy.sh --frontend` |
...
| Команда | Назначение |
|---------|------------|
| `./scripts/dev.sh` | Локальная разработка (Docker + Vite) |
| `./scripts/deploy.sh` | Деплой на Pi (API + фронт) |
| `./scripts/deploy-backend.sh` | Только API |
| `./scripts/deploy-frontend.sh` | Только сайт |
| `./scripts/internal/sync-db-from-pi.sh` | Полный дамп БД с Pi → локально |
| `./scripts/internal/sync-db-to-pi.sh --confirm` | Локальная БД → Pi (перезапись) |

**Редко (`scripts/internal/`):** `tunnel-db.sh` — PostgreSQL с Pi → `127.0.0.1:5432`. Новые лекарства — **`rx-import.sql`** в консоли БД (`rx_import_medicines`). Не **`import.sh`** — восстановит архивный список.
...
| `setup/intellij-docker-ssh.sh` | **`roster-docker-auto`** — Docker via SSH в JetBrains (LAN/VPS) |
| `setup/intellij-db-ssh.sh` | **`roster-db-auto`** — PostgreSQL на Pi через SSH в JetBrains |
