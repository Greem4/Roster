...
Импорт лекарств из JSON: `./scripts/internal/import.sh`.
...
**Редко (`scripts/internal/`):** `tunnel-db.sh` — PostgreSQL с Pi → `127.0.0.1:5432` (если нужен только туннель); `import.sh` — перезаливка лекарств из JSON.
