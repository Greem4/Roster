...
Cursor-агент после правок **API** может запустить `deploy-backend.sh` для быстрой проверки; **фронт на сайт** — только по запросу.

**Один раз дома** — см. [SSH на Pi](#ssh-на-pi-дома-и-вне-дома) и [Mac: команды → setup](#mac-команды).

**Вне дома** (кафе, LTE): SSH на Pi — **`ssh roster-pi-remote`**, не `greem4@192.168.31.96`. Деплой — `./scripts/deploy-backend.sh` (маршрут auto). Локальная разработка — Docker на Mac, без туннеля к API. Подробно — ниже ↓
...
| Команда | Назначение |
|---------|------------|
| `./scripts/dev.sh` | UI локально (API — локальный Docker, иначе Pi/prod) |
| `./scripts/internal/dev-stack.sh` | Локальный Docker: db + api |
...
| `./scripts/deploy-backend.sh` | Код API на малинку |
